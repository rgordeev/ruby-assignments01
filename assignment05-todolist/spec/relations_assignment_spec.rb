...
feature "Relations Assignment" do
  ...
  context "rq03" do # Re-use and update 'Profile' model and database table
    context "Profile Model:" do
      ...
      it "Profile database structure in place" do
        # rails g model profile gender birth_year:integer first_name last_name photo
        # rake db:migrate
        expect(Profile.column_names).to include "gender", "birth_year", "first_name", "last_name", "photo"
        ...
        expect(Profile.column_types["photo"].type).to eq :string
        expect(User.column_types["created_at"].type).to eq :datetime
        expect(User.column_types["updated_at"].type).to eq :datetime
      end
  ...
  context "rq07" do
      ...
      context "rq07e" do
        scenario "Successful login results in navigation to list items page" do
          visit(login_path)
          fill_in "user[username]", with: "Clinton"
  ...
  context "rq11" do
    context "Validators:" do
      ...
      it "does not allow a Profile with a null first and last name and photo" do
        expect(Profile.new(:first_name=>nil, :last_name=>nil, :photo=>nil, :gender=>"male")).to_not be_valid
      end
      it "does not allow a Profile with a null photo" do
        expect(Profile.new(:first_name=>"name", :last_name=>"lastname", :photo=>nil, :gender=>"male")).to_not be_valid
      end
      it "allows a Profile with a null first name when last name present" do
        expect(Profile.new(:first_name=>nil, :last_name=>"Smith", :photo=>"path/to/photo", :gender=>"male")).to be_valid
      end
      it "allows a Profile with a null last name when first name present" do
        expect(Profile.new(:first_name=>"Joe", :last_name=>nil, :photo=>"path/to/photo", :gender=>"male")).to be_valid
      end

      it "does not allow a Profile with a gender other than male or female " do
        expect(Profile.new(:first_name=>"first", :last_name=>"last", :photo=>"path/to/photo", :gender=>"neutral")).to_not be_valid
      end
      it "does not allow a boy named Sue" do
        expect(Profile.new(:first_name=>"Sue", :last_name=>"last", :photo=>"path/to/photo", :gender=>"male")).to_not be_valid
      end
      it "allows a Profile with gender male" do
        expect(Profile.new(:first_name=>"first", :last_name=>"last", :photo=>"path/to/photo", :gender=>"male")).to be_valid
      end
      it "allows a Profile with gender female" do
        expect(Profile.new(:first_name=>"first", :last_name=>"last", :photo=>"path/to/photo", :gender=>"female")).to be_valid
      end
    end
  end
  ...
  context "check database operations" do
    ...
    context "rq12" do
      ...
      it "User delete must cascade delete to Profile, TodoList and TodoItems" do
        user = User.create(:username=>"testUser", :password_digest=>"xxxx")
        profile = Profile.create(:first_name=>"Joe", :last_name=>"Smith", :photo=>"path/to/photo", :birth_year=>2000, :gender=>"male", :user_id=>user.id)
        tdlist = TodoList.create(:list_name=>"my list", :list_due_date=>Date.today, :user_id=>user.id)
    ...
    context "rq14 - Profile" do
      ...
      it "will return a list of profiles between requested birth years in ascending order" do
        ...
        (0..20).each do |i|
          ...
          profile = Profile.create(:user_id=>user.id, :gender=>"male", :photo=>"path_to_photo", :birth_year=>birthYear, :first_name=>"User #{i}", :last_name=>"Smith#{i}")
        end
